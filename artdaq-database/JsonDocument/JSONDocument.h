...
public:
  explicit notfound_exception( std::string const& category_):
      cet::exception( category_ ){}
};

class readonly_exception : public cet::exception {
public:
  explicit readonly_exception( std::string const& category_):
      cet::exception( category_ ){}
};
...
public:  
   ...
   template<typename T>
    T  value_as(path_t const& path) const {return boost::lexical_cast<T>(findChild(path).value());}
