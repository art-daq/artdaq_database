...
template <typename CONF> struct ConfigurationInterface final {
  std::string _database_provider;

  ConfigurationInterface(std::string const &database_provider)
      : _database_provider(database_provider){};

  //==============================================================================
  // stores configuration version to database
  template <typename TYPE>
  cf::result_pair_t storeVersion(CONF configuration, std::string const &version,
                                 std::string const &entity) const noexcept try {
    ...
    auto serializer =
        ConfigurationSerializer<CONF, MakeSerializable>::wrap(configuration);
    ...
    if (std::is_same<TYPE, JsonData>::value) {
      ...
      throw artdaq::database::invalid_option_exception(apifunctname)
          << "Unsupported storage format " << typeid(TYPE).name()
          << ",  use either JsonData or FhiclData.";
    }

    if (!entity.empty())
      opts.configurableEntity(entity);
    ...
    if (!apiCallResult.first)
      throw artdaq::database::runtime_exception(apifunctname)
          << apiCallResult.second;
  ...
  template <typename TYPE>
  cf::result_pair_t loadVersion(CONF configuration, std::string const &version,
                                std::string const &entity) const noexcept try {
    assert(!version.empty());
  ...
  std::list<std::string> getVersions(CONF configuration,
    ...
    try {
      ...
      if (!jsn::JsonReader().read(apiCallResult.second, resultAST))
        throw artdaq::database::runtime_exception(apifunctname)
            << "Invalid JSON:" << apiCallResult.second;
      try {
        auto const &searches =
            boost::get<jsn::array_t>(resultAST.at(cfl::document::search));

        for (auto const search : searches) {
          auto const &query =
              boost::get<jsn::object_t>(search).at(cfl::document::query);
          auto const &filter =
              boost::get<jsn::object_t>(query).at(cfl::filterx);
          auto const &version = boost::get<std::string>(
              boost::get<jsn::object_t>(filter).at(cfl::filter::version));
          returnList.emplace_back(version);
        }
      } catch (std::exception const &e) {
        throw artdaq::database::runtime_exception(apifunctname)
            << "Unable to read versions from  JSON:" << apiCallResult.second
            << "; Exception:" << e.what();
      }
    } catch (std::exception const &e) {
      returnList.clear();
      throw artdaq::database::runtime_exception(apifunctname) << "Exception:"
                                                              << e.what();
    } catch (...) {
      returnList.clear();
      throw artdaq::database::runtime_exception(apifunctname)
  ...
  std::list<std::string>
  findGlobalConfigurations(std::string const &mongosearch) const {
    auto returnList = std::list<std::string>{}; // RVO

    constexpr auto apifunctname =
        "ConfigurationInterface::findGlobalConfigurations";
    ...
    try {
      ...
      if (!mongosearch.empty())
        opts.globalConfiguration(mongosearch);
      ...
      try {
        ...
        for (auto const search : searches) {
          ...
          auto const &filter =
              boost::get<jsn::object_t>(query).at(cfl::filterx);
          auto const &configuration = boost::get<std::string>(
              boost::get<jsn::object_t>(filter).at(cfl::filter::configuration));
          returnList.emplace_back(configuration);
        }
      } catch (std::exception const &e) {
        throw artdaq::database::runtime_exception(apifunctname)
            << "Unable to read configuration names from  JSON:"
            << apiCallResult.second << "; Exception:" << e.what();
      }
    } catch (std::exception const &e) {
      returnList.clear();
      throw artdaq::database::runtime_exception(apifunctname) << "Exception:"
  ...
  //==============================================================================
  //
  std::list<VersionInfo>
  loadGlobalConfiguration(std::string const &configuration) const {
    assert(!configuration.empty());

    constexpr auto apifunctname =
        "ConfigurationInterface::loadGlobalConfiguration";

    auto returnList = std::list<VersionInfo>{}; // RVO
    ...
    try {
      ...
      if (!configuration.empty())
        opts.globalConfiguration(configuration);
      ...
      try {
        ...
        for (auto const search : searches) {
          ...
          auto const &version = boost::get<std::string>(
              boost::get<jsn::object_t>(filter).at(cfl::filter::version));
          auto const &entity = boost::get<std::string>(
              boost::get<jsn::object_t>(filter).at(cfl::filter::entity));
          auto const &configuration = boost::get<std::string>(
              boost::get<jsn::object_t>(filter).at(cfl::filter::configuration));
        ...
        throw artdaq::database::runtime_exception(apifunctname)
            << "Unable to read version info names from  JSON:"
            << apiCallResult.second << "; Exception:" << e.what();
      }
  ...
  //
  cf::result_pair_t
  storeGlobalConfiguration(std::list<VersionInfo> const &versionInfoList,
                           std::string const &configuration) const try {
    assert(!configuration.empty());
    assert(!versionInfoList.empty());

    constexpr auto apifunctname =
        "ConfigurationInterface::storeGlobalConfiguration";

    if (versionInfoList.empty())
      throw artdaq::database::invalid_option_exception(apifunctname)
          << "Version info list is empty";

    if (configuration.empty())
      throw artdaq::database::invalid_option_exception(apifunctname)
          << "Global configuration name is empty";

    auto payloadAST = jsn::object_t{};
    payloadAST[cfl::operation::operations] = jsn::array_t{};
    auto &operations =
        boost::get<jsn::array_t>(payloadAST[cfl::operation::operations]);
    ...
    for (auto const &versionInfo : versionInfoList) {
      ...
      op[cfl::option::format] = data_format_t::gui;
    ...
    if (!jsn::JsonWriter().write(payloadAST, buffer))
      throw artdaq::database::runtime_exception(apifunctname)
          << "JsonWriter failed, invalid payloadAST.";
