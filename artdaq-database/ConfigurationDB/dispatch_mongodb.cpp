...
JsonData prov::configurationComposition(ManageDocumentOperation const& options, JsonData const& search_payload) {
  ...
  TLOG(18) << "operation_confcomposition: configurationComposition returned " << query_payloads.size() << " results.";
...
void cf::debug::MongoDB() {
  ...
  TLOG(0) <<  "artdaq::database::configuration::MongoDB trace_enable";
}
