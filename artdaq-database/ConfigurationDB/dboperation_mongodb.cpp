...
JsonData cfd::findConfigVersions(cfo::LoadStoreOperation const& options, JsonData const&) {
  assert(options.provider().compare(cfo::literal::database_provider_mongo) == 0);
...
JsonData cfd::findConfigEntities(cfo::LoadStoreOperation const& options, JsonData const&) {
  assert(options.provider().compare(cfo::literal::database_provider_mongo) == 0);
...
JsonData cfd::addConfigToGlobalConfig(cfo::LoadStoreOperation const& options, JsonData const& search_payload) {
  ...
  find_options.provider(cfo::literal::database_provider_mongo);
