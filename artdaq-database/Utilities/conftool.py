...
def __allow_importing_incomplete_configurations():
    try:
        allow_incomplete = os.environ[
            "ARTDAQ_DATABASE_ALLOW_INCOMPLETE_CONFIGURATIONS"
        ].lower()
...
def __increment_config_name(config):
    ...
    match = re.match(r"(.*[^\d])(\d+$)", config)
    return (
        match.group(1) + str(int(match.group(2)) + 1).zfill(5)
        if match
        else config + str(1).zfill(5)
    )
...
def __latest_config_name(configNamePrefix):
    ...
    if __ends_on_5digitnumber(configNamePrefix):
        ...
        configs = [
            ...
            if re.match(r"(^" + configNamePrefix + r")(\d{5}$)", config)
        ]
...
def __validate_schema(schema):
    ...
    for l in ["artdaq_processes", "artdaq_includes", "system_layout", "run_history"]:
        if l not in schema:
            print(
                "Error: Invalid schema.fcl; A top level " + l + " sequence is missing."
            )
            errors_count += 1
            continue
        ...
        if isinstance(schema[l], list) is not True:
            print(
                "Error: Invalid schema.fcl; A top level "
                + l
        ...
        for d in schema[l]:
            ...
            if isinstance(d, dict) is not True:
                print(
                    "Error: Invalid schema.fcl; A top level "
                    + l
                    + " is not a sequence of tables type."
                )
                errors_count += 1
                break
            ...
            if "collection" not in d or "pattern" not in d:
                print(
                    error_message
                    + 'is missing either "collection" or "pattern" key-value pairs.'
                )
                errors_count += 1
                continue
    ...
    if errors_count > 0:
        ...
        print(
            "Info: Replaced schema.fcl with the default one; review schema.fcl and re-run the import command."
        )
        exit(1)
...
def getListOfAvailableRunConfigurations(searchString="*"):
    query = json.loads('{"operation" : "findconfigs", "dataformat":"gui", "filter":{}}')
    query["filter"]["configurations.name"] = (
        searchString if searchString.endswith("*") else searchString + "*"
    )
...
def __getListOfMaskedRunConfigurations(flags, useMask=True):
    if not artdaq_database_uri.startswith("mongodb://"):
        print(
            "Warning: __getListOfMaskedRunConfigurations is not implemented; returning an empty result."
        )
        return {}

    query = json.loads(
        '{"operation" : "searchcollection", "collection":"Flags","dataformat":"json", "filter":{"pipeline":[] } }'
    )
    ...
    pipeline.append(
        json.loads(
            '{"$project":{"_id": 0, "config":"$configurations.name","version":"$version"}}'
        )
    )
    pipeline.append(json.loads('{"$unwind":{"path":"$config"} }'))
    pipeline.append(json.loads('{"$sort":{ "config": 1, "version": 1}}'))
    pipeline.append(
        json.loads('{"$group":{"_id": "$config","version": { "$last": "$version" }}}')
    )
    pipeline.append(
        json.loads('{"$project":{"_id": 0, "config":"$_id","version":"$version"}}')
    )
    pipeline.append(json.loads('{"$sort":{ "config": 1, "version": 1}}'))
...
def __exportConfiguration(config):
    ...
    for special_file in special_files:
        for cfg in cfgs:
            if (
                cfg[entity_name] == special_file[0]
                and cfg[collection] == special_file[1]
            ):
                ...
                with open(file_name, "w") as fcl_file:
                    ...
                    if sys.version_info[0] < 3:
                        cfg_entry = (
                            cfg_entry[0].encode("utf-8"),
                            cfg_entry[1].encode("utf-8"),
                            cfg_entry[2].encode("utf-8"),
                        )
    ...
    for cfg in cfgs:
        ...
        path = (
            cfg[collection] if cfg[collection] in include_dirs else __get_prefix(config)
        )
...
def __importConfiguration(
    configNameOrconfigPrefix, update=False, updateFlagsOnly=False
):
    configPrefix = __get_prefix(configNameOrconfigPrefix)

    if configNameOrconfigPrefix == configPrefix and update is True:
        print(
            "Error: Updates are not allowed using the configuration prefix; configNameOrconfigPrefix=",
            configNameOrconfigPrefix,
        )
    ...
    if update:
        ...
        if version in found_versions or updateFlagsOnly:
            ...
            version = (
                match.group(1) + str(int(match.group(2)) + 1).zfill(3)
                if match
                else config + "-ver" + str(1).zfill(3)
            )
            print("Updating configuration", config)
            print("Info: Storing version " + version)
        else:
            print(
                "Error: Configuration update failed; configuration "
                + configNameOrconfigPrefix
                + " is not found."
            )
            return False
        ...
        config = (
            __increment_config_name(config)
            if config
            else __increment_config_name(configPrefix)
        )
        version = config + "-ver001"
    ...
    cfg_composition = list(
        __composition_reader(
            ...
            schema,
            __find_files(".", fcl_file_pattern),
        )
    )

    excluded_files = __list_excluded_files(
        __get_prefix(config),
        schema,
        __find_files(".", any_file_pattern),
        cfg_composition,
    )

    if len(excluded_files) > 0:
        print(
            "Warning: The following files will be excluded from being loaded into the artdaq database "
            + ", ".join(excluded_files)
            + ". Update "
            + fhicl_schema
            + " to include them."
        ...
        if not __allow_importing_incomplete_configurations():
            print(
                "Error: Importing of incomplete configurations is not allowed;"
    ...
    cfg_composition_dict = dict(
        zip(iter([cfg[1] for cfg in cfg_composition]), iter(cfg_composition))
    )
...
def __exportConfigurationWithBulkloader(config):
    ...
    setenvvars_command = " ".join(
        'export {}="{}";'.format(k, v)
        for k, v in {k: envvars[k] for k in keys if k in envvars}.items()
    )
    setenvvars_command += 'export ARTDAQ_DATABASE_URI="{}";'.format(
        __archiveuri_from_uri(envvars["ARTDAQ_DATABASE_URI"])
    )
    setenvvars_command += "echo BULKDOWNLOADER is running on $(hostname -s) and ARTDAQ_DATABASE_URI=$ARTDAQ_DATABASE_URI"

    data_files_dir = (
        os.getcwd() if artdaq_database_remote_host is None else "$bulkloader_data_dir"
    )
    ...
    bulkloader_command = (
        "bulkdownloader -r {} -c {} -p {} -t $(( $(nproc)/2 ))\nrc=$?".format(
    ...
    scp_command2 = "scp {} -r $bulkloader_data_dir/* {}:{}".format(
        sshopts, socket.gethostname(), os.getcwd()
    )
    ...
    if artdaq_database_remote_host is None:
        command = ";".join(
            [setenvvars_command, cddir_command, bulkloader_command, retcode_command]
        )
    else:
        command = "ssh {} {} '{}'".format(
            artdaq_database_remote_host,
            sshopts,
            ";".join(
                [
                    setenvvars_command,
    ...
    try:
        ...
        print(
            "Error: Failed running bulkdownloader over ssh on "
            + artdaq_database_remote_host
            + ". Exception message: "
            + str(e)
        )
...
def exportConfiguration(configNamePrefix):
    ...
    if config is None:
        print(
            "Error: Configuration %s is not present in the database." % configNamePrefix
        )
        return False
...
def __archiveConfigurationWithBulkloader(config, run_number, update):
    ...
    bulkloader_command = (
        "bulkloader -r {} -c {} -p {} -t $(( $(nproc)/2 ))\nrc=$?".format(
            run_number, config, data_files_dir
        )
    )
    retcode_command = "[[ $rc -eq 0 ]] && echo True || echo False"
    ...
    scp_command += "scp {} -r {}:{} $bulkloader_data_dir".format(
        sshopts, socket.gethostname(), os.getcwd()
    )
    ...
    try:
        ...
        print(
            "Error: Failed running bulkloader over ssh on "
            + artdaq_database_remote_host
            + ". Exception message: "
            + str(e)
        )
...
def listDatabases():
    result = conftoolp.list_databases(
        '{"operation" : "listdatabases", "dataformat":"csv", "filter":{}}'
    )
    if result[0] is not True:
        __report_error(result)
        return None
...
def listVersions(collection_name):
    query = json.loads(
        '{"operation" : "findversions", "dataformat":"csv", "filter":{}}'
    )
...
def listCollections():
    result = conftoolp.list_collections(
        '{"operation" : "listcollections", "dataformat":"csv", "filter":{}}'
    )
    if result[0] is not True:
        __report_error(result)
        return None
...
def listConfigurations():
    result = conftoolp.find_configurations(
        '{"operation" : "findconfigs", "dataformat":"csv", "filter":{}}'
    )
    if result[0] is not True:
        __report_error(result)
        return None
...
def readDatabaseInfo():
    result = conftoolp.read_dbinfo(
        '{"operation" : "readdbinfo", "dataformat":"json", "filter":{}}'
    )
    if result[0] is not True:
        __report_error(result)
        return None
...
def exportDatabase():
    ...
    result = conftoolp.export_database(
        '{"operation" : "exportdatabase", "dataformat":"json", "source": ".",  "filter":{}}'
    )
    if result[0] is not True:
        __report_error(result)
...
def importDatabase():
    print("ARTDAQ_DATABASE_URI", os.environ["ARTDAQ_DATABASE_URI"])
    result = conftoolp.import_database(
        '{"operation" : "importdatabase", "dataformat":"json", "source": ".", "filter":{}}'
    )
    if result[0] is not True:
        __report_error(result)
...
def help():
    ...
    print(
        " conftool.py getListOfAvailableRunConfigurationsSubtractMasked  {} #file with configuration flags".format(
            flags_file
        )
    )
    print(
        " conftool.py getListOfMaskedRunConfigurations {} #file with configuation flags".format(
            flags_file
        )
    )
...
if __name__ == "__main__":
    ...
    if sys.argv[1] in [
        o[0]
        for o in inspect.getmembers(sys.modules[__name__])
    ...
    functions_list = [
        o[0]
        for o in inspect.getmembers(conftoolp)
        if inspect.isfunction(o[1]) and not o[1].__name__.startswith("_")
    ]
