...
result_pair_t load_configuration( std::string const& search_filter ) {
    try {
        if ( search_filter.empty() )
            return std::make_pair( false, make_error_msg( "Search filter is empty" ) );
...
result_pair_t store_configuration( std::string const& search_filter, std::string const& json_document ) {
    try {
        ...
        if ( json_document.empty() )
            return std::make_pair( false, make_error_msg( "JSON document is empty" ) );
...
bool test_storeconfig(std::string const& config_name, std::string const& json_name)
{
    ...
    std::string search_filter((std::istreambuf_iterator<char>(is)),
    ...
    std::string json_document((std::istreambuf_iterator<char>(is1)),
                              std::istreambuf_iterator<char>());
