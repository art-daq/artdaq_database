...
std::string dbfs::mkdir(std::string const& d) {
  ...
  TLOG(11)<< "StorageProvider::FileSystemDB mkdir dir=<" << dir << ">";
  ...
  if (boost::filesystem::exists(boost::filesystem::status(path))) {
    if (boost::filesystem::is_directory(path)) {
      ...
      if ((boost::filesystem::status(path).permissions() & mask) != mask) {
        TLOG(11)<< "StorageProvider::FileSystemDB Directory  <"
                       << dir << "> has wrong permissions; needs to be owner readable and writable";
        throw runtime_error("FileSystemDB") << "Directory  <" << dir
                                            << "> has wrong permissions; needs to be owner readable and writable";
      }
    } else {
      TLOG(11)<< "StorageProvider::FileSystemDB Failed creating a directory, sometging in the way path=<" << dir
                                                                                                          << ">";
  ...
  if (!boost::filesystem::create_directories(path, ec)) {
    if (ec != boost::system::errc::success) {
      TLOG(11)<< "StorageProvider::FileSystemDB Failed creating a directory path=<" << dir
                                                                                    << "> error code=" << ec.message();
      throw runtime_error("FileSystemDB") << "Failed creating a directory path=<" << dir
                                          << "> error code=" << ec.message();
    }
  }
  ...
  /*
   ...
   boost::filesystem::permissions(path, perms, ec);
    ...
    if (ec != boost::system::errc::success) {
      TLOG(11)<< "StorageProvider::FileSystemDB Failed enforcing directory permissions for path=<" << dir << "> error
   code=" << ec.message());
...
std::list<std::string> dbfs::find_subdirs(std::string const& d) {
  ...
  TLOG(11)<< "StorageProvider::FileSystemDB find_subdirs dir=<" << dir << ">";

  auto path = boost::filesystem::path(dir.c_str());

  if (!boost::filesystem::exists(boost::filesystem::status(path))) {
    TLOG(11)<< "StorageProvider::FileSystemDB Failed searching for subdirectories, directory does not exist path=<"
                   << dir << ">";
  ...
  if (!boost::filesystem::is_directory(path)) {
    TLOG(11)<< "StorageProvider::FileSystemDB Failed searching for subdirectories, not a directory path=<" << dir
                                                                                                           << ">";
  ...
  for (boost::filesystem::directory_iterator dir_iter(path); dir_iter != end_iter; ++dir_iter) {
    ...
    TLOG(11)<< "StorageProvider::FileSystemDB Found subdirectory, path=<" << dir_iter->path().filename().string()
                                                                          << ">";
...
std::list<object_id_t> dbfs::find_documents(std::string const& d) {
  ...
  TLOG(11)<< "StorageProvider::FileSystemDB find_documents dir=<" << dir << ">";

  auto path = boost::filesystem::path(dir.c_str());

  if (!boost::filesystem::exists(boost::filesystem::status(path))) {
    TLOG(11)<< "StorageProvider::FileSystemDB Failed searching for documents, directory does not exist path=<" << dir
                                                                                                               << ">";
  ...
  if (!boost::filesystem::is_directory(path)) {
    TLOG(11)<< "StorageProvider::FileSystemDB Failed searching fo documents, not a directory path=<" << dir << ">";
  ...
  for (boost::filesystem::directory_iterator dir_iter(path); dir_iter != end_iter; ++dir_iter) {
    ...
    TLOG(11)<< "StorageProvider::FileSystemDB Found document, oid=<" << file.string() << ">";
...
std::list<std::string> dbfs::find_siblingdirs(std::string const& d) {
  ...
  TLOG(11)<< "StorageProvider::FileSystemDB find_siblingdirs dir=<" << dir << ">";
...
file_paths_t dbfs::list_files_in_directory(boost::filesystem::path const& path, std::string const& ext[[gnu::unused]]) {
  ...
  if (!boost::filesystem::exists(boost::filesystem::status(path))) {
    TLOG(11)<< "StorageProvider::FileSystemDB Directory does not exist, path=<" << path.string() << ">";
