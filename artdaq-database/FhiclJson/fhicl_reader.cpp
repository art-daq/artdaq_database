...
bool FhiclReader::read_data_gui(std::string const& in, jsn::array_t& json_array) {
  ...
  try {
    ...
    for (auto const& atom : fhicl_table) array.push_back(fcl2jsongui(atom, atom, comments));
...
bool FhiclReader::read_includes(std::string const& in, jsn::array_t& json_array) {
  ...
  try {
    ...
    for (auto const& include : includes) array.push_back(include);

    if (result) json_array.swap(array);
