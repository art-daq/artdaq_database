...
fcl2jsongui::fcl2jsongui(fhicl_key_value_pair_t const& self_, fhicl_key_value_pair_t const& parent_,
                         comments_t const& comments_)
    : self{self_}, parent{parent_}, comments{comments_} {}

fcl2jsongui::operator jsn::object_t() try {
  ...
  switch (value.tag) {
    ...
    case ::fhicl::SEQUENCE: {
      ...
      for (auto const& tmpVal : fcl_value::sequence_t(value)) tmpArray.push_back(dequote(fcl_value::atom_t(tmpVal)));
    ...
    case ::fhicl::TABLE: {
      ...
      for (auto const& kvp : fcl_value::table_t(value)) tmpArray.push_back(fcl2jsongui(kvp, self, comments));
...
json2fclgui::json2fclgui(jsn::value_t const& self_, jsn::value_t const& parent_) : self{self_}, parent{parent_} {}

json2fclgui::operator fcl::value_t() try {
...
json2fclgui::operator fcl::atom_t() try {
  ...
  switch (type) {
    ...
    case ::fhicl::SEQUENCE: {
      ...
      try {
        ...
        for (auto const& val : values) sequence.push_back(json2fclgui(val, self));
    ...
    case ::fhicl::TABLE: {
      ...
      try {
        ...
        for (auto const& val : values) table.push_back(json2fclgui(val, self));
