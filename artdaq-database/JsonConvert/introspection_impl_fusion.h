...
template <typename Flavour, typename UserData> struct VisitorApplication;
...
struct members_impl<Visitor, S, typename size<S>::type> {
    static inline void handle(Visitor&, S&) {
        /*Nothing to do*/
    }
};
...
struct array_application {
    ...
    static inline void handle(Visitor& visitor, UserData& array) {
        ...
        for (auto & element : array)
            VisitorApplication<Visitor, value_type>::handle(visitor, element);
...
struct value_application<Visitor, optional<UserData> > {
    ...
    static inline void handle(Visitor& visitor, optional<UserData>& data) {
        if (data)
            VisitorApplication<Visitor, UserData>::handle(visitor, *data);
        //  else
...
struct select_application {
    ...
    typename boost::mpl::eval_if < detail::is_vector<UserData>, identity<array_application <Visitor, UserData>>,
             ...
             identity<value_application<Visitor, UserData>>
             > >::type type;
};
