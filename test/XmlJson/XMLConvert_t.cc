...
{
  artdaq::database::xml::trace_enable_XmlReader();
  artdaq::database::xml::trace_enable_XmlWriter();
  artdaq::database::xmljson::trace_enable_xml2json();
  artdaq::database::xmljson::trace_enable_xmljsondb();
  ...
  debug::registerUngracefullExitHandlers();
    ...
    auto runTest = [](std::string const & name) {
        auto tests = std::map<std::string, test_case> {
            {"xml2json", test_convertxml2json},
            {"json2xml", test_convertjson2xml}
        };
