...
//    artdaq::database::fhicljson::useFakeTime(true);
     ...
     desc.add_options()
     ( "source,s", bpo::value<std::string>(),  "Configuration source file name." )
     ( "compare,c", bpo::value<std::string>(),  "Expected return file name." )
     ( "filter,f", bpo::value<std::string>(), "Search filter file name." )
...
bool test_storeconfig( std::string const& filt, std::string const& src, std::string const& cmp ) {
     ...
     std::string filt_json( ( std::istreambuf_iterator<char>( is1 ) ),
                            std::istreambuf_iterator<char>() );
     is1.close();

     std::ifstream is2( src );
     std::string src_json( ( std::istreambuf_iterator<char>( is2 ) ),
                           std::istreambuf_iterator<char>() );
     is2.close();

     std::ifstream is3( cmp );
     std::string cmp_json( ( std::istreambuf_iterator<char>( is3 ) ),
                           std::istreambuf_iterator<char>() );
     is3.close();
